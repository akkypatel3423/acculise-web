    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-white text-center">
        <div>
           ...
           <Link href="/industries" className="btn-primary inline-block">Back to Intelligence Board</Link>
        </div>
      </div>
    )
  }
              ...
              <div className="flex flex-col lg:flex-row items-center gap-16">
                <AnimatedSection className="flex-1 text-center lg:text-left">
                   <div className="inline-flex items-center space-x-3 mb-8 px-5 py-2 bg-white/5 backdrop-blur-xl border border-white/10 rounded-full">
                      <span className="text-[20px] leading-none">{industry.icon}</span>
                      <span className="text-blue-300 font-black uppercase tracking-[0.4em] text-[10px]">Vertical Specialist</span>
                   </div>
                   <h1 className="text-5xl md:text-8xl xl:text-9xl font-black text-white leading-[0.8] tracking-tighter mb-10 italic">
                      ...
                      <span className="text-secondary underline decoration-white/10 decoration-8 underline-offset-4">{industry.title.split(' ').slice(1).join(' ')}</span>
                   </h1>
                   ...
                   <div className="flex flex-wrap justify-center lg:justify-start gap-4">
                      {industry.solutions.slice(0, 2).map((s, i) => (
                        <span key={i} className="px-6 py-3 bg-white/5 border border-white/10 rounded-full text-white text-[11px] font-bold uppercase tracking-widest backdrop-blur-md">
                          {s}
                        </span>
                      ))}
                   </div>
                ...
                <AnimatedSection delay={0.2} className="hidden lg:block w-[400px] h-[400px] relative">
                   ...
                   <div className="absolute inset-0 bg-white/5 backdrop-blur-3xl rounded-[4rem] border border-white/10 shadow-2xl flex flex-col items-center justify-center p-12 text-center group hover:-rotate-3 transition-transform duration-700">
                      <div className="text-7xl mb-6 transform group-hover:scale-110 transition-transform">{industry.icon}</div>
                      <p className="text-blue-200 font-black uppercase tracking-[0.3em] text-xs">Sector Precision</p>
                 ...
                 <div className="lg:col-span-7 space-y-24">
                    <AnimatedSection>
                       <div className="flex items-center space-x-3 mb-10">
                          ...
                          <span className="text-[11px] font-black uppercase tracking-[0.5em] text-primary">Technical Strategy</span>
                       </div>
                       ...
                       <div className="prose prose-slate prose-xl max-w-none text-slate-600 leading-relaxed font-medium space-y-8">
                          ...
                          <p>
                             Our {industry.title} desk isn't just about accounting—it's about building a technical moat for your business. We analyze the unique pressure points of your vertical, from supply chain logistics to complex revenue recognition standards.
                          </p>
                       </div>
                    </AnimatedSection>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                       <AnimatedSection delay={0.1}>
                          <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-2xl shadow-blue-900/5 h-full group hover:bg-slate-900 transition-colors duration-500">
                             <div className="w-12 h-12 rounded-full bg-red-50 text-red-500 flex items-center justify-center mb-10 font-black group-hover:bg-red-500 group-hover:text-white transition-colors tracking-tighter text-xs">ERR</div>
                             <h4 className="text-[11px] font-black text-slate-400 uppercase tracking-widest mb-6 group-hover:text-slate-500 transition-colors">Sector Friction</h4>
                             <ul className="space-y-6">
                                {industry.challenges.map((c, i) => (
                                   <li key={i} className="flex items-start text-slate-700 font-bold tracking-tight group-hover:text-slate-300 transition-colors">
                       ...
                       <AnimatedSection delay={0.2}>
                          <div className="bg-primary p-12 rounded-[3.5rem] text-white h-full shadow-2xl relative overflow-hidden group">
                             ...
                             <div className="w-12 h-12 rounded-full bg-white/10 text-secondary flex items-center justify-center mb-10 font-black tracking-tighter text-xs border border-white/5">SOL</div>
                             <h4 className="text-[11px] font-black text-blue-300 uppercase tracking-widest mb-6 relative z-10">Acculise Engine</h4>
                             <ul className="space-y-6 relative z-10">
                                {industry.solutions.map((s, i) => (
                                   <li key={i} className="flex items-start text-blue-50 font-bold tracking-tight italic">
                                      <svg className="w-4 h-4 text-secondary mt-1 mr-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><path d="M5 13l4 4L19 7"/></svg>
                                      {s}
                                   </li>
                                ))}
                             </ul>
                    ...
                    <div className="sticky top-32 space-y-8">
                       <AnimatedSection>
                          <div className="card-modern p-12 md:p-16 border-none shadow-2xl shadow-blue-900/10 bg-white group overflow-hidden relative">
                             ...
                             <h4 className="text-3xl font-black text-slate-900 mb-8 uppercase tracking-tighter italic">Precision Audit.</h4>
                             <p className="text-slate-500 text-lg font-medium mb-12 leading-relaxed">
                                Our sector-specific board provides the technical backbone your business needs to expand without friction across US and UK corridors.
                             </p>
                             ...
                             <Link href="/contact" className="btn-primary w-full shadow-2xl py-6 hover:scale-105">Request Sector Blueprint</Link>
                             <p className="mt-8 text-center text-[10px] font-black text-slate-300 uppercase tracking-[0.4em]">Reference Code: {industry.slug.toUpperCase()}</p>
                          </div>
                       </AnimatedSection>
                       
                       <AnimatedSection delay={0.3}>
                          <div className="p-10 rounded-[3rem] bg-slate-900 text-white shadow-xl relative overflow-hidden">
                             <p className="text-[10px] font-black text-blue-400 uppercase tracking-[0.3em] mb-4">Market Update</p>
                             <p className="text-blue-100 font-bold italic leading-relaxed">
                                Our technical board is currently monitoring {industry.title} regulatory shifts for the 2026 fiscal cycle.
                             </p>
                          </div>
                       </AnimatedSection>
                    </div>
