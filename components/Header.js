    ...
    <header
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled
          ...
          : "bg-transparent py-6 bg-white/90"
      }`}
    >
      <nav className="container-custom flex items-center justify-between">
        <Link href="/" className="flex items-center space-x-2 group">
          ...
          <div className="flex flex-col">
            <span
              className={`text-xl font-bold tracking-tight ${isScrolled ? "text-primary" : "text-primary"}`}
            >
              Acculise
            </span>
            <span className="text-[10px] text-gray-500 font-bold tracking-widest uppercase flex items-center">
              Solutions{" "}
              <span className="ml-1 w-1 h-1 bg-accent rounded-full animate-pulse"></span>
            </span>
          </div>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-10">
          {navLinks.map((link) => (
            ...
            >
              {link.name}
              <span
                className={`absolute -bottom-1 left-0 w-full h-0.5 bg-primary scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left ${pathname === link.href ? "scale-x-100" : ""}`}
              ></span>
            </Link>
          ))}
        </div>
